...
const MDNWSCreatorCorrection = ({ exo, onMDNDataChange }) => {
    ...
    function getMDN() {
        if (exo.length !== 0) {
            ...
            let reponseFinal = exo.reponseFinal
            console.log(reponseFinal)
            let nom = exo.nom
            let texte = "<div id='mdn_resulat'>"
            let score = 0;
            for (let i = 0; i < length; i++) {
                texte += '<input class="answerUserMDN" value=' + reponseFinal[score] + '>' + '</input><input class="answerUserMDN" value="' + reponseFinal[score + 1] + '"></input>'
                texte += "<br></br>"
                score += 2;
            }

            texte += "</div>"
            document.getElementById("zoneExoMDNCorrection").innerHTML = texte
        ...
        <div>
            {exo.length !== 0 ? (
                <div id="div_mdn">
                    ...
                    <br />
                    <div id="zone_abaque">
                        <div id="zoneExoMDNCorrection"></div>
                    </div>
                </div>
            ) : (
                <div>
...
export default MDNWSCreatorCorrection;