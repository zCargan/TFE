...
const AbaqueWSCreatorCorrection = ({ exo, onAbaqueDataChange }) => {
    ...
    function recupereExo() {
        ...
        if (exo.length !== 0) {
            ...
            for (let i = 0; i < hauteur; i++) {
                for (let j = 0; j < longueur; j++) {
                    if ((reponseInitiale[k]) == undefined) {
                        texte += "<input  ici' class='inputUserResponseAbaque'></input>"
                    } else {
                        texte += "<input  ici' class='inputUserResponseAbaque'" + " value='" + resultatAttendu[k] + "'></input>"
                    }
            ...
            document.getElementById("abaqueFromDBCorrection").innerHTML = texte;
        }
    }
...
export default AbaqueWSCreatorCorrection;